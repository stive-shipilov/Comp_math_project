...
- **`src/comp_math/differentiation/`** - модуль численного дифференцирования и расчёта якобиана
...
- **`src/comp_math/nonlinear/solvers/`** - содержит реализации методов решения СЛАУ:
  - **`iterative/`** - итерационные методы (Бисекции, Простой итерации, метод Ньютона)
  - **`variational/`** - вариационные методы
- **`src/comp_math/interpolation/impl/`** - содержит реализации методов интерполяции (метод Ньютона, кубических сплайнов, МНК):
- **`src/comp_math/integration/impl/`** - содержит реализации  методы численного интегрирования (метод прямоугольников, трапеций, симпсона, квадратур Гаусса, Монте-Карло):
