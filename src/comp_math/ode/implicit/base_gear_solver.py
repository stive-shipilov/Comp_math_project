        ...
        while t_current < t_span[1]:
            ...
            try:
                ...
                if len(y) <= self.order:
                    ...
                    # TODO: заменить на мою реализацю, так сейчас мои не справляются с 
                    # большими числами в матрице
                    y_new_np = np.linalg.solve(M, rhs)
            ...
            # Адаптивное уменьшение щага, на случай, если переданного h не хваьтило 
            except Exception:
                current_h *= 0.5
                if current_h < 1e-12:
                    break
